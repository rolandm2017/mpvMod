...
"""
LibMPV-based time monitor with all features from the IPC version.
Uses native libmpv for better performance instead of JSON IPC.

Usage:
- python libmpv_time_monitor.py "path/to/video/file.mp4"
- Or run without arguments and load files via MPV window
"""

class LibMPVTimeMonitor:
    def __init__(self, poll_interval=0.208):
        ...
        # Create MPV instance with same settings as IPC version
        self.player = mpv.MPV(
            idle=True,  # Keep player open when no media
        ...
        # Set up event handlers and property observers
    ...
    def setup_event_handlers(self):
        ...
        def on_pause_observer(_name, value):
            # Track pause state changes to avoid duplicate messages
...
def main():
    # Example usage
    monitor = LibMPVTimeMonitor(poll_interval=0.208)  # 208ms intervals
    ...
    if len(sys.argv) > 1:
        ...
        print("   - Load a file: python libmpv_monitor.py 'path/to/video.mp4'")
        print("   - Or load files manually in the MPV window")
        print("   - Or drag and drop files into the MPV window")
    
    print("\n🎮 Controls:")
    print("   Ctrl+C: Quit")
    print("   Close MPV window: Automatically stops monitoring")
    print("   Use MPV window for playback controls")
    print("   Native libmpv performance (no IPC overhead)")
